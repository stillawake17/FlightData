...
def get_flights_departure_data(airport_icao, start_time, end_time):
    url = f"{BASE_URL}/departure?airport={airport_icao}&begin={start_time}&end={end_time}"
...
def save_to_daily_file(data):
    ...
    # Including a 'd' in the filename for departures
    filename = f"flights_departure_data_d_{datetime.datetime.now().strftime('%Y_%m_%d')}.json"
...
flights_departure_data = get_flights_departure_data(AIRPORT_ICAO, start_time, end_time)
save_to_daily_file(flights_departure_data)